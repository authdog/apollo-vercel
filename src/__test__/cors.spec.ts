...
describe(`skip node v6 and v14`, () => {
  ([6, 14].includes(parseInt(process.versions.node.split(`.`, 1)[0], 10))
    ? describe.skip
    ...
    it(`allows for uploading multiple files`, async () => {
      ...
      const res = await request(app)
        ...
        .field(
          `map`,
          JSON.stringify({ 0: [`variables.files.0`], 1: [`variables.files.1`] })
        )
        .attach(`0`, `package.json`)
...
describe(`cors`, () => {
  ...
  it(`should add expected cors headers to response`, async () => {
    ...
    expect(res.header[`access-control-allow-credentials`]).toStrictEqual(
      `true`
    );
    expect(res.header[`access-control-allow-headers`]).toStrictEqual(
      `origin,content-type`
    );
    expect(res.header[`access-control-allow-methods`]).toStrictEqual(
      `GET,POST`
    );
  });
});
